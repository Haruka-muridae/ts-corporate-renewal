...
### 1-1. 「(Google Meet)」の併記を外した（2026-08-08・オーナー指示）

確定稿からの変更。照合に使う文字列も差し替えている（件数は 70 のまま）。

| 箇所 | 変更前 | 変更後 |
| --- | --- | --- |
| 料金カードの条件行 | 月4回 × 60分｜オンライン**(Google Meet)**｜参加人数の追加料金なし | 月4回 × 60分｜オンライン｜参加人数の追加料金なし |
| FAQ 1 の回答（本文） | …オンライン会議**(Google Meet)**で接続できる範囲で… | …オンライン会議で接続できる範囲で… |
| FAQ 1 の回答（JSON-LD） | 同上 | 同上 |

**FAQ は本文と JSON-LD の両方を直した。** 片方だけだと Google はスパムとして
扱う（下の照合で 4/4 一致を確認）。

「月4回の進め方」カード02 の **「Google Meetで実施。実際にAIを操作しながら
進めます。」は残している。** ここは「オンライン（Google Meet）」という表記
ではなく、消すと文が成立しない（新しい文言を作ることになる）ため。
オーナー確認済み。

#### 折り返しへの影響（副次的に改善した）

料金カードの条件行は `text-wrap: balance` の対象。文字列が短くなったことで
折り位置が変わった。

| 幅 | 変更前 | 変更後 |
| --- | --- | --- |
| 320px | 3行。**「オンラ / イン(GoogleMeet)｜」と語の途中で割れる** | 2行 |
| 375px | 2行。**「オンライン(Google / Meet)｜」と製品名が割れる** | 2行 |
| 768px 以上 | 1行 | 1行 |

> **320px・375px では区切り「｜」が2行目の行頭に来る**
> （「月4回×60分｜オンライン / ｜参加人数の追加料金なし」）。
> 変更前より読みやすくなってはいるが、完全ではない。直すには文言か
> マークアップに手を入れる必要があるため、**指示の範囲を超えると判断して
> そのままにしてある。**
...
### 4-1. モバイルの明示改行（2026-08-08・オーナー指定）

`.br-sp`（600px 以下だけ出る）と `.br-pc`（601px 以上だけ出る）を足し、
7箇所の折り位置を指定した。**文章のテキストは1文字も変えていない**
（`<br>` の挿入と、申込み見出しの `br-lg` → `br-pc` だけ）。

> **1つの `<br>` に `br-lg` と `br-sp` を併記していない。** どちらが効くかが
> CSS の記述順で決まってしまうため、両方要る箇所は `<br>` を2つ並べている。
> 同時に出ることはない（768px 以上／600px 以下で排他）。

375px の実測。行の切れ目は `getBoundingClientRect()` の `top` で
グループ化して復元している（目視ではなく機械測定）。

| # | 対象 | 指定どおりか |
| --- | --- | --- |
| 1 | Hero見出し | **○** 社員全員を、/ AIを使える人材に。 |
| 2 | Hero説明文 | **△** 3段目が二重に折り返る（下記） |
| 3 | 課題見出し | **○** 3セグメントとも1行 |
| 4 | 価値メッセージ | **○** 3セグメントとも1行 |
| 5 | 料金構造見出し | **○** 3セグメントとも1行 |
| 6 | 料金構造本文 | **△** 1・2段目が二重に折り返る（下記） |
| 7 | 申込み見出し | **○** 明示改行なし。自然折り返しで2行 |

#### 375px で1行に収まらなかったセグメント

**フォントサイズは変えていない**（オーナー判断待ち）。

| 箇所 | セグメント | 実際の折り |
| --- | --- | --- |
| 2 Hero説明文 | チーム全体で、AIを業務で使える状態を目指します。 | 「…状態を目指 / します。」 |
| 6 料金構造本文 | 一般的な企業研修は「1人あたり◯円」の課金です。 | 「…の課金 / です。」 |
| 6 料金構造本文 | 本プログラムは月額55,000円(税込)の固定制。 | 「…の固定 / 制。」 |

#### `text-wrap: balance` との干渉

`balance` が効いているのは `.h-hero` `.h-section` `.value-message` `.price-terms`
の4つ。**375px では各セグメントが1行に収まるため、balance の出番が無く干渉しない。**
二重折り返しが出た2と6は、そもそも `balance` を当てていない要素
（`.hero-lead` `.pricing-body`）で、素の折り返しがそのまま出ている。

320px（AGENTS.md の下限）では Hero見出しが
「社員全員を、/ AIを使える人材 / に。」となり、**`balance` が効いているのに
「に。」が1行に残る。** 明示改行でセグメントに分かれると、Chrome は
セグメントをまたいで行数を均さないため。**指定は 375px なので直していない。**

#### デスクトップ側への影響

768px と 1440px は、**変更前と行の切れ目が完全に一致**することを確認した
（変更前の `index.html` / `style.css` を取り出して同じ測定を実行）。

**601〜767px だけ、申込み見出しが1行 → 2行に変わる。** `br-pc` の境界が
601px で、`br-lg` の 768px より広いため。指示どおりの結果だが、
**この帯だけ挙動が変わる**ことは記録しておく。
...
| 3回目 | **`210b6b26-0544-46ba-9fe3-ce29764be7bc`** | `f5d2f85`（正式公開・POTENITAS 導線・相互リンク） | **6ファイル**（両LPの `index.html` と `css/style.css`・`potenitas/index.html` ＋ `BUILD_ID`） |
| 4回目 | **`6107eb2c-e589-4ba3-903a-cdc50c346b68`** | `0c6c90c`（モバイル改行） | **15ファイル**（法人LPの `index.html` と `css/style.css` ＋ `BUILD_ID`。残りは `main` に先に入っていたショート動画 台本メーカー一式） |
| **5回目（現行）** | **`2663fd42-ff0c-47b4-b235-01584731f203`** | `4b63212`（「(Google Meet)」の併記を外す） | **2ファイル**（`index.html` ＋ `BUILD_ID`） |

切り戻し先の版: `218a6e77-6eea-4487-bdcd-0d97074a4f74`（2回目）。
そこからさらに戻すなら `f2aabd2c`（1回目）→ `566a82b4`（法人LP追加前）。

> **3回目の7分後（2026-08-07T23:03Z）に、別のデプロイ `df0d00c9-bf9f-4abc-9f1c-bfd791ef3230`
> が入っている。** LP の作業とは無関係で、`main` に後から入った
> ショート動画 台本メーカー（`d1558cc`）を含む版。**現行はこちら。**
> LP 側の配信物は 3回目と変わっていないことを、配信物とリポジトリの
> 突き合わせで確認済み（下表）。**LP を切り戻すときは、この版を巻き込む
> ことになる点に注意する。**
...
上表の確認は、後続デプロイ `df0d00c9` が入ったあとにもう一度実行して
同じ結果（配信物 vs リポジトリ 5点とも差分0行・収束 75/75・回帰5ページ同一）
になることを確かめてある。**LP の配信内容は後続デプロイでも変わっていない。**

### 8-4. 4回目（モバイル改行・2026-08-08）の確認

| 確認 | 結果 |
| --- | --- |
| 配信物 vs リポジトリ（法人LP の HTML と CSS・個人LP・POTENITAS） | **4点とも 差分 0 行** |
| 本番 375px の行の切れ目 | **ローカルの測定と完全一致**（§4-1 の表） |
| 本番URLでの再計測 | コントラスト79/79・axe 0件・横スクロール3幅なし・reduced-motion 38/38・FAQ 4/4 |
| 相互リンクの往復 | 両方向とも 200・同一タブ（デスクトップ／モバイル） |
| 回帰7ページ | **内容は全件一致**（下記の改行コードの注記を参照） |
| 付随アセット（両LPの css / js / favicon / ogp） | **8点とも 200** |
| `/event/apply/` | 200 かつ `x-opennext: 1` |
| キャッシュ収束 | 1ラウンド目で 40/40。追加2ラウンドも 50/50・50/50 |

**ロールバックは発生していない。**

> **このデプロイには、`main` に先に入っていたショート動画 台本メーカーも
> 含まれている。** LP の作業とは無関係だが、`main` を出す以上は同時に出る。
> アップロード15件の内訳は LP 2件・`BUILD_ID` 1件・台本メーカー12件。

#### 改行コードがビルドしたマシンで入れ替わる

**配信される静的HTMLの改行コードが、デプロイのたびに CRLF ⇄ LF で入れ替わる。**

```
デプロイ前（Windows でビルドされた版）… / は CRLF。43279 → 44032 バイト
デプロイ後（WSL でビルドした版）      … / は LF
中身は1文字も違わない（改行を揃えると完全一致）
```

リポジトリの作業ツリーは CRLF、WSL 側のクローンは LF でチェックアウトされる
ためで、**表示にも動作にも影響はない。** ただし次の2点に効く。

- **「1バイトも変わらないこと」で回帰を見る方法が、ビルド機が変わった回だけ
  使えない。** 改行を揃えてから比較すること（このレポートの回帰確認もそうした）。
- Cloudflare は内容ハッシュで重複排除するため、**改行が変わった回は全ファイルが
  「新規」扱いになりうる。** 今回15件で済んだのは、LF 版が3回目のデプロイで
  既にアップロード済みだったため（バケットには過去の版の実体も残っている）。

### 8-5. 5回目（「(Google Meet)」の併記を外す・2026-08-08）の確認

アップロードは **2ファイル**（`index.html` ＋ `BUILD_ID`）のみ。同梱物なし。

| 確認 | 結果 |
| --- | --- |
| 配信物 vs リポジトリ（法人LP・CSS・個人LP・POTENITAS） | **4点とも 差分 0 行** |
| 本番の `オンライン(Google Meet)` | **0件** |
| 本番の `オンライン会議(Google Meet)` | **0件** |
| 本番の `Google Meet` 言及数 | **1件**（進め方カード02 のみ＝意図どおり） |
| 本番URLでの再計測 | コントラスト79/79・axe 0件・横スクロール3幅なし・reduced-motion 38/38・**FAQ 4/4**（本文と JSON-LD が一致） |
| 回帰7ページ | **全件 バイト完全一致**（前回と同じ WSL ビルドなので改行も揃っている） |
| キャッシュ収束 | 1ラウンド目から 25/25。追加2ラウンドも 25/25・25/25 |
