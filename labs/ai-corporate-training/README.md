...
| **2026-08-08** | **「(Google Meet)」の併記を外す（3箇所）** | オーナー指示。確定稿からの変更なので、`qa/verification-report.md` §1 の照合文字列も差し替えた。**FAQ は本文と JSON-LD の両方**を直している（片方だけ直すとスパム扱いになる）。「Google Meetで実施。」（月4回の進め方カード02）は文が成立しなくなるため**残した**（オーナー確認済み） |
