...
| 2026-08-07 | **FAQ に Eyebrow と見出しを置かない** | 実装指示書 4.9 が指定していないため。**指示漏れの可能性がある**（下記） |
...
- [ ] **FAQ セクションに Eyebrow と見出しを付けるか**

  実装指示書 4.9 は、他のセクションと違って Eyebrow も見出しも指定していない。
  「書かれていないものを足さない」という指示に従い**置いていない**が、
  他の全セクションに Eyebrow があるため、**指示漏れの可能性が高い。**

  付けるなら、姉妹LPと同じ形で `index.html` の FAQ セクション冒頭へ2行足す。

  ```html
  <p class="eyebrow reveal">FAQ</p>
  <h2 class="h-section reveal" id="faq-title">よくある質問。</h2>
  ```

  併せて `<section class="section">` に `aria-labelledby="faq-title"` を足す。
  **見出しの文言は確定稿に無いため、こちらで決めずに残している。**

- [ ] **法人導入相談の予約ページを用意し、`href="#"` を差し替える**

  差し替えるのは申込みセクションの1箇所だけ（該当行に TODO コメントあり）。
  ヘッダー・Hero・料金カードの3つは `#apply` へのページ内アンカーのままでよい。

- [ ] **`noindex` を外す**（予約ページ接続後）
