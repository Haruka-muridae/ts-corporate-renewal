...
7. body                            ← ブラウザUIへ抜ける
8. a: 申し込む                      ← 先頭へ戻る
9. a: マンツーマンAI学習を始める      ← Hero CTA
```

**ヘッダーCTA → 本文リンク → FAQ の順で到達できる。** DOM 順どおりで、
`tabindex` による順序の作り替えは行っていない。

> **申込みセクションはフォーカス対象を持たない。** 申込み受付が準備中のあいだ、
> CTA ボタンを置かず「申し込み受付は、現在準備中です。」という文章にしているため
> （押せないボタンを作らないという判断）。ヘッダーと料金カードの `#apply` アンカーで
> 移動すれば、その文章が読める。フォーム完成後に CTA を戻すと、FAQ の次に
> 申込みCTAが入る形へ戻る。
...
```
確定稿 49 件中 49 件一致
確定稿を差し引いた残り: "パーソナルAIトレーニング申し込み受付は、現在準備中です。"
```
...
| 残った文字列 | 理由 |
| --- | --- |
| `パーソナルAIトレーニング` | サイト名がヘッダーとフッターの両方に出るため、1回分しか引けていない |
| `申し込み受付は、現在準備中です。` | **今回意図的に追加した1文**（申込みフォーム未完成のため） |

**したがって、指示書の確定稿に無い日本語の地の文は、準備中の1文のみである。**
