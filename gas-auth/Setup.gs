...
/**
 * コールドスタート回避のための定期実行（時間主導トリガーから呼ぶ）。
 *
 * ------------------------------------------------------------------
 * 何もしない関数では足りない
 * ------------------------------------------------------------------
 * Apps Script はしばらく呼ばれないと停止し、次の起動に時間がかかる。
 * 実測で `?action=health` の初回に 35 秒かかったことがある。
 * ログイン画面はこの往復を待つため、そのまま利用者の待ち時間になる。
 *
 * ここで空の関数を回しても、温まるのはスクリプトの実行環境であって、
 * **Web アプリとして配信される経路とは限らない**。そこで自分自身の
 * /exec を叩き、利用者が通るのと同じ道を実際に通しておく。
 *
 * 効果は Google 側の都合に依存しており、**保証されたものではない**。
 * 設定後に実際のログイン時間を測って確かめること。
 * ------------------------------------------------------------------
 *
 * ------------------------------------------------------------------
 * 例外を投げない
 * ------------------------------------------------------------------
 * 5分おきに走る関数が失敗すると、実行ログが失敗で埋まり、
 * 本当に見たい障害が埋もれる。取れなかった場合は理由を返して終わる。
 * ------------------------------------------------------------------
 *
 * 対象は参照系の health だけで、シートを読み書きしない。
 */
function keepAlive() {
  var url = '';
  ...
  try {
    var response = UrlFetchApp.fetch(url + '?action=health', {
      method: 'get',
