 ...
 * 複製元
 * ------------------------------------------------------------------
 * workers/notifier-gate/scripts/generate-vapid-keys.mjs（2026-08-26 に複製）。
 * docs/repository-structure.md §4-1 に従い、共通層を作らず写した。
 * 変えたのは参照先の config パスだけ。
 *
 * **notifier-gate とは別の鍵ペアを使う。** 同じ鍵を共有すると、
 * 片方をローテーションしたときにもう片方の購読まで無効になる。
 * ------------------------------------------------------------------
 *
 * ------------------------------------------------------------------
 * openssl を使わない理由
 ...
 * 鍵を差し替えると全利用者の購読が無効になる（workers/push-assistant/README.md §4）。
 * ------------------------------------------------------------------
 */
...
console.log('登録コマンド:');
console.log('  wrangler secret put VAPID_PRIVATE_KEY --config workers/push-assistant/wrangler.jsonc');
console.log('  wrangler secret put VAPID_PUBLIC_KEY  --config workers/push-assistant/wrangler.jsonc');
