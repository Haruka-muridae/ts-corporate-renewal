...
Apps Script API の**生の応答を実行ログへ出す**ブロックを入れてある
（`[一時デバッグ]` で始まる行）。2026-08-11 に、公開が 403 で止まる原因が
「利用者設定」なのか「GCP プロジェクトで API 未有効」なのか切り分けられず、
本文を捨てていたことが分かったため。

**恒久的に残すものではない。**

- 応答の本文をそのまま4000文字まで出しており、Google 側の文面が変われば
  何が出るか分からない。**出す内容を自分で決めていない**のがよくない
- 原因の切り分けは、同じコミットで入れたエラー種別
  （`API_DISABLED` / `API_DISABLED_GCP`）と、Google が示す有効化URLの
  抽出（`firstHttpsUrl_`）で足りるようにしてある

**やること**: 原因が確定したら、`scriptApiFetch_` の
`★一時デバッグ★` のコメントごとブロックを消す。
種別ごとのログ（`deployFailure_` が出している1行）は残す。
[tests/unit/notifier-template.mjs](../tests/unit/notifier-template.mjs) の
「★失敗時に生の応答をログへ出す」の検査も、あわせて外す。

**なぜ今やらないか**: まだ原因が確定していない。
確定するまでは、この出力が唯一の手掛かりである。

**着手条件**: 403 の原因が確定し、公開が通った時点。
テンプレートを貼り替える次の機会に、同じ貼り替えでまとめて行う。
