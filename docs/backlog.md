...
### B-06. 管理者メールが未取得ドメインのアドレスになっている（優先度: 高）

**内容**: 認証システムの初期管理者アドレスが `architect@potenitas.com`
（[gas-auth/Config.gs](../gas-auth/Config.gs) の `INITIAL_ADMIN_EMAIL`、
法務文書の連絡先 `LEGAL_CONTACT_EMAIL` も同じ）になっているが、
**`potenitas.com` は当社が取得していないドメインである。**

危険なのは「メールが届かない」ことではなく、**第三者がこのドメインを取得すると
管理者宛のメールを受信できてしまう**ことである。届く先は次のようなものになる。

- パスワード初期設定リンク・再設定リンク（[gas-auth/Mailer.gs](../gas-auth/Mailer.gs)）
- 管理者アカウントに紐づく各種の案内

つまり **管理者アカウントの乗っ取り経路**になりうる。「そのうち取る」で
放置してよい種類の問題ではない。

**やること**: どちらかを行う。

1. 管理下のドメインのアドレスへ変更する（`INITIAL_ADMIN_EMAIL` と
   `LEGAL_CONTACT_EMAIL`、および法務文書スプレッドシートの連絡先）
2. `potenitas.com` を取得する

**取得も変更も済むまでは、管理者メール経由のフローを使わない運用とする。**
具体的には、管理者アカウントのパスワード再設定をメールリンクで行わず、
Apps Script のエディタから直接操作する。

**なぜ今やらないか**: カレンダー通知 V2 の実装とは独立しており、
どちらの手段を採るか（ドメインを買うか、別アドレスにするか）が
運営側の判断だから。**ただし優先度は高く、V2 の完了を待つ理由は無い。**

**着手条件**: 即時。遅くとも、管理者アカウントでパスワード再設定を
行う必要が生じる前。
