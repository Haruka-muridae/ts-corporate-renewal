...
**11本での再測定は、各アプリの詳細設計書が揃った時点で行う。この節は現時点では未確定である。**
いま言えるのは次の2点だけ。

1. §3-7 の通信部と §3-1 のローダは、2本比較のときより**同一である可能性が高い**
   （6本・5本に増え、コメント上の差分が小さい）
2. §3-4 の `valueInputOption` のような**方針の食い違い**は、共通化するなら
   1つずつ決着させる必要がある。2026-08-04 の調査では13点あった

**この節を根拠に共通層を作らないこと。** 判断には11本すべての設計書と、
[../instructions/2026-08-18-app-design-docs-handoff.md](../instructions/2026-08-18-app-design-docs-handoff.md) §2 の
方針決定（A／B／C）が要る。
