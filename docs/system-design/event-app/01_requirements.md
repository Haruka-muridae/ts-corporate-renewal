...
| ID | 要件 | 補足 |
| --- | --- | --- |
| FR-01 | 告知ページ（静的HTML）に開催概要・参加費のレンジ・キャンセルポリシーを表示する | `public/event/index.html`。開催日時はDBとは別に静的HTML側にも保持する（§9参照） |
...
- **開催日時が `public/event/index.html`（静的HTML）と `events.event_date` の2箇所に存在する。** 次回開催時は両方を更新する。詳細ページをNext化しない限り解消しない。
