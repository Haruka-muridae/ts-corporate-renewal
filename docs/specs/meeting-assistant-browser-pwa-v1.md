...
2. 認証チェックポイント（有効で残り 10 分以上）を通れば、保存先フォルダを保証 → Drive へアップロード → 文字起こし・議事録 → Markdown 保存 → **全工程の成功を確認してから**台帳と OPFS から削除（`save-flow.js`。2026-08-25 改訂: 議事録の処理が終わるまで OPFS の録音を消さない。要件定義書 v1.1 §4-4 / v1.2 §5）。録音開始時に Google 連携は求めない。
3. 未連携なら台帳に残してホームへ戻す。ホームの「処理が終わっていない録音があります」から **「Driveへ保存」**（利用者の押下＝ポップアップを開ける文脈）で連携してから保存する。失敗した行は「前回の保存に失敗」と表示し「Driveへ再送」になる。「破棄」で端末から削除できる。
4. 起動時の OPFS 掃除（`cleanupStaleFiles`）は台帳に載っているファイルを残し、それ以外の `.part` だけ消す。台帳にあるのに OPFS に無い行は落とす。

補助:

- **録音開始時に Google 連携は求めない**（2026-08-25 改訂、要件定義書 v1.2）。停止後の保存直前に認証チェックポイント（有効で残り 10 分以上）を通し、足りなければ台帳に残してホームへ戻す。「Driveへ保存」の押下（利用者操作の猶予がある文脈）で連携を更新してから保存する。
