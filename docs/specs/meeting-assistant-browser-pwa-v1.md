...
2. Google 連携済み（トークン有効）なら、Drive へアップロード → 文字起こし・議事録 → Markdown 保存 → **全工程の成功を確認してから**台帳と OPFS から削除（`save-flow.js`。2026-08-25 改訂: 議事録の処理が終わるまで OPFS の録音を消さない。要件定義書 v1.1 §4-4）。
