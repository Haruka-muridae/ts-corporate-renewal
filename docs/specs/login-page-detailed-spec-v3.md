...
- 到達経路: 直接アクセス / guardPage() によるリダイレクト(`?next=<画面名>` 付き) / ログアウト後遷移
- 成功時の遷移先: `?next=` の画面名(許可リスト照合後)。現行は Portal と録音アプリの2つ(§6)
...
| 2 | 見出し(h1) | 「ログイン」 |
| 3 | 導入文 | 1文以内 |
| 4 | メールアドレス入力 | `type="email"`、label 必須、`autocomplete="username"` |
...
| 8 | メッセージ領域 | エラー `role="alert"` / 案内 `role="status"`。初期は空。**位置は remember とログインボタンの間**(送信前にエラーが読み上げられ、ボタンまでのフォーカス移動を短く保つため) |
...
- パラメータは `?next=<画面名>` と、**その画面ごとに許可したクエリ**のみ
- 許可リスト ALLOWED_NEXT(session.js)は `['portal', 'voiceRecorder']`
...
- guardPage() は元URLのパス・ハッシュを引き継がない。クエリは下表のものだけを引き継ぐ

#### 画面ごとの引き継ぎクエリ(NEXT_PARAM_RULES / session.js)

| next 画面名 | 引き継ぐクエリ | 受け付ける形 | 用途 |
| --- | --- | --- | --- |
| `portal` | なし | — | — |
| `voiceRecorder` | `eventId` | `^[A-Za-z0-9_-]{1,512}$` | カレンダー通知から開いた予定の表示(録音アプリ 5.3) |

- 検証は `safeNextParams(nextName, source)` が行う。**形に合わない値は落とす**(丸めない・直さない)
- 引き継ぎ経路は3か所とも同じ関数を通る: `goToLogin()` / `goToScreen()` / `redirectIfSignedIn()`
- ログイン画面は `readNextParams()` で読み、成功時に `goToScreen(nextName, nextParams)` へ渡す

**この方式を採った経緯**: 未ログイン状態でカレンダー通知をクリックすると、ログイン後に `?eventId=` が失われ、どの予定の通知だったのかが画面に出せなかった(2026-08-09 の実機検証で確認)。元URLをそのまま持ち回る方式(`/apps/` 側の `safeNextUrl` 相当)は、上記の恒久原則に反するため採らず、下の「将来拡張」3 の方式で実装した。

### 将来拡張(発動条件つき)

**保護対象画面が3つを超えた時点で**、次の再設計を行う:

1. ALLOWED_NEXT を SCREENS 定義(config.js)から導出し、画面追加時の多重修正(page / session / config)を解消する — **未実施**
2. 画面名ベースの元画面復帰(`?next=<画面名>` を guardPage が自動設定)を導入する — **未実施**
3. クエリ・ハッシュの復元が必要になった場合は、画面ごとに許可パラメータを個別検証する方式とする — **2026-08-09 実施**(上表)

いずれの場合も「任意URLを受け取らない」原則は維持する。保護対象ページを追加する際の現行手順: ①ページで `guardPage({ next: '<画面名>' })` を明示指定 ②ALLOWED_NEXT へ追加 ③SCREENS 定義を確認。クエリを引き継ぐ場合は ④NEXT_PARAM_RULES へ名前と正規表現を追加。

> **1 と 2 の発動条件はすでに満たされている。** 現在の保護対象画面は portal / card-ocr(+help, +measure) / receipt-ocr / short-script(+help) / voice-recorder で、3つを超えている。ただし 1・2 は全アプリの遷移先が変わる変更であり、カレンダー通知の不具合修正とは切り離す。着手時は各アプリの `guardPage({ next: ... })` を一斉に見直すこと。
...
- [ ] `?next=voiceRecorder&eventId=...` がログイン後に復元される(形に合わない eventId は落ちる)
- [ ] 許可していない画面のクエリは引き継がれない(`?next=portal&eventId=...` で eventId が消える)
...
ブラウザ録音アプリのカレンダー通知にあわせて改訂する。
未ログイン状態で通知をクリックすると、ログイン後に `?eventId=` が失われ、
対象の予定を表示できなかった(実機検証で確認)。

| # | 節 | 変更 |
|---|----|------|
| 1 | §6 | ALLOWED_NEXT へ `voiceRecorder` を追加。`?next=portal` 固定の記述を改めた |
| 2 | §6 | 画面ごとの引き継ぎクエリ(NEXT_PARAM_RULES)を新設。`voiceRecorder` の `eventId` のみ |
| 3 | §6 | 「将来拡張」1・2 は未実施であること、発動条件はすでに満たされていることを明記 |
| 4 | §1 | 到達経路と遷移先の記述を上記に合わせた |
| 5 | §12 | 受け入れテストへ引き継ぎクエリの項目を追加 |

**「任意URLを受け取らない」恒久原則は維持している。** 元URLをそのまま
持ち回る方式(`public/apps/shared/app-paths.js` の `safeNextUrl` 相当)は
採用せず、画面ごとに名前と形を許可する方式(将来拡張3)で実装した。
...
v3 と実装の突き合わせ検証で判明した4件の不一致について裁定した結果を反映する。
...
あわせて §13 の既知の課題2件(文言の二重定義 / ストレージ利用不可文言の直書き)を解消済みへ更新した。
