...
起案: 2026年8月3日 / 更新: 2026年8月3日（§6 の6論点を決定、Cloud作業の完了とクォータ実測を反映）
...
**未記入の2項目**（確認でき次第ここへ追記する）:

| 項目 | 状態 |
| --- | --- |
| 「未確認アプリ」警告の有無 | **未記入。** 公開ステータスが「テスト」のため出る可能性がある。MVP公開前に「本番」へ切り替える際の判断材料になる |
| 同意画面に出たスコープの文言 | **未記入。** `drive.file` であれば「このアプリで使用」「開いた」といった限定が入るはずで、限定の無い表現なら設定の誤りを疑う |

> **この2つは推測で埋めない。** 実際に画面で見た内容だけを書くこと。
...
| 状態 | 項目 |
| --- | --- |
| **合格** | 0-1（§4-0）、**0-2**、**1〜8**（§4-0-1）、16（§4-3） |
| 実行できる | 9・10・11（PoCへ実装済み。プレビューで実行するのみ） |
| サンプル待ち | 12・13 |
| 未着手 | 14、15 |
| 記録待ち | 「未確認アプリ」警告の有無、同意画面のスコープ文言（§4-0-1） |
