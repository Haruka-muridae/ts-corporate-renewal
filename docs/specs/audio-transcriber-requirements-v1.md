...
2. Gemini モードを使う場合、利用者が Google AI Studio で自分のAPIキーを
   取得していること。アプリはキーを保存しない。
...
| APIキーの保存（次回も使える） | キーを端末へ残さない方針（§10）と矛盾する |
...
3. モードを選ぶ（既定は端末内処理）。Gemini モードならAPIキーを貼り付ける。
...
## 10. APIキーの扱い（利用者の Gemini キー）

- 保持は `script.js` のモジュールスコープ変数1つだけ。
  localStorage / sessionStorage / Cookie / URL / DOM属性 / Drive へ書かない。
  console へ出さない。エラー文言にも混ぜない。
- ページを閉じるか再読み込みすれば消える。共有端末では使用後に
  ページを閉じるよう画面に明記する。
- 入力欄は `type="password"` + `autocomplete="off"`（ブラウザの
  パスワード保存ダイアログを出さない）。
...
| 脅威 | 対策 |
| --- | --- |
| トークン・APIキーの漏えい | メモリ上のみ保持（§FR-02 / §10） |
