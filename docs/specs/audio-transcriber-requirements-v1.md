...
| サービス | 用途 | 備考 |
| --- | --- | --- |
| Google Identity Services | OAuth 認可（トークンモデル） | 「Googleドライブから選択」押下時のみ |
...
- Google 連携（Drive）は「Googleドライブから選択」または「Googleドライブへ保存」を
...
3. 「音声ファイルを選ぶ」を開き、「端末から選択」または
   「Googleドライブから選択」を押す（**両方とも常に出ている**。FR-04）。
...
- **両方の取得ボタンを常に表示する。**片方を `hidden` で隠さない。
  隠す方式では、端末が既定のときに Google ドライブから選ぶ手段が画面から消え、
  設定を開いて切り替えないと使えなかった（実機で発覚。§12）。
- **強調の差（端末＝主要ボタン／Googleドライブ＝補助ボタン）は固定とし、
  利用者に選ばせない**（v1.4）。以前は「よく使う入力元」という設定で強調を
  入れ替えていたが、切り替えても押せる操作は変わらず、変わるのは見た目だけ
  だった。設定項目・説明文・実際に取得できた入力元の記憶処理を持つ対価に
  見合わないため、まとめて廃止した（§12）。
- 取得元の説明は各ボタンの直下に1つだけ置き、対象フォルダのパスは
  「ドライブ全体は検索しない」ことを述べる注記と、Drive選択ダイアログの
  見出しにだけ出す（同じパスを同一画面へ3回書かない）。
...
- **結果操作の並びで最も強いトーン（塗りつぶし）を持つボタンは1つだけとする**
  （v1.4）。「AI議事録を作成」（FR-09）は次のアプリへ進む導線であって
  この画面の完了操作ではないため、補助トーンに置く。並び順は
  Drive保存 → コピー → TXT保存 → AI議事録を作成 → 結果をクリアとし、
  作業を捨てる操作を末尾かつ最も弱いトーンにする。
...
**文言の書き方**（v1.4。本番アプリ共通の基準）

- Google ドライブは画面上の日本語表記を「Googleドライブ」に揃える
  （「Google Drive」と混在させない）。Drive保存ボタンの文言は
  「Googleドライブへ保存」、選択ボタンは「Googleドライブから選択」。
  API そのものを指す「Google Drive API」だけは製品名としてそのまま使う。
- 説明文は1段落に3文以上詰めない。趣旨の単位で段落を分け、必要なら
  `<br>` で文単位に改行する（interview-recorder の開始画面と同じ流儀）。
- **同じことを同じ画面で2度書かない。**注記を足す前に、既存の注記・
  状態表示で同じ内容を言っていないかを確かめる。
- 「当社サーバーへ送らない」旨は、データの流れが同じ箇所では
  「データは当社のサーバーを経由せず、当社が内容を受け取ることは
  ありません。」に揃える。ただし Gemini モードのように実際の送信先が
  ある場合は**正確さを優先**し、まとめて保証しない。
...
v1.3 まではここに「よく使う入力元（端末 / Google ドライブ）」があったが、
FR-04 の設定ごと廃止した（v1.4）。**既存の利用者の保存値には
`fileSource` キーが残る**ため、読み込み側は知らないキーを読み捨てるだけとし、
値が入っていても他の設定の復元を妨げないこと（単体テストで固定する）。
...
- **入力元を設定で1つに絞り、選ばれた側だけを表示する**: 実機で不成立と
  ...
  設定を開いて切り替えないとドライブが使えなかった。**両方を常に出す**
  方式へ改めた（FR-04）。
- **「よく使う入力元」の設定（v1.3 まで存在）**: 上の改修の残り物として、
  「どちらのボタンを強調するか」だけを決める設定が残っていた。
  切り替えても使える手段は変わらず、変わるのは塗りつぶし／ゴーストの
  見た目だけで、それを説明するために「設定」と「音声ファイルを選ぶ」の
  両方へ注記（「どちらもいつでも使えます」等）が必要になっていた。
  設定1つのために画面の文字数が増える構図だったため、**設定・注記・
  取得できた入力元の記憶処理をまとめて廃止**し、強調は固定にした（v1.4）。
  「両方を常に出す」という上の結論は維持している。
- **Gemini モードカードの「APIキーが必要です。実環境によって利用できない
  場合があります。」の注記**: 前半は同じカードの説明文（利用者自身の
  Gemini APIキーを使う）と重複し、後半は FR-11 の接続状態表示が
  実際の利用可否を理由付きで出すようになった時点で役目を終えていた。
  v1.4 で削除した（§13-5 の確認事項も同時に改めた）。
...
4. 本番URLで「Googleドライブから選択」を押し、認可が通ること・
   録音アプリの録音が一覧に出ることを実アカウントで確認すること
   （テスト版はローカルオリジンでのみ確認済み）。
5. Gemini モードの実APIキーでの通信確認（テスト版から未確認のまま）。
   **v1.4 で画面の「実環境によって利用できない場合があります」の注記は
   削除済み**（FR-11 の接続状態表示が実際の利用可否を理由付きで出すため。
   §12）。この項目は「注記を外すための確認」ではなく、
   公開前に実キーで疎通を確かめること自体として残す。
...
| 1.4 | 2026-08-19 | **認知負荷の見直し（機能は変えず、削る／畳む／文言統一のみ）。** ①「よく使う入力元」の設定を廃止し、取得ボタンの強調を固定にした（FR-04・FR-10・§12。両方を常に出す結論は維持）②結果操作で最も強いトーンのボタンを1つに絞り、「AI議事録を作成」を補助トーンへ降格・並び順を整理（FR-07）③ Gemini モードカードの「実環境によって利用できない場合があります」の注記を削除（§12・§13-5。FR-11 の接続状態表示が役目を引き継いでいる）④画面上の「Google Drive」表記を「Googleドライブ」へ統一し、説明文の段落分けと重複注記の除去の基準を FR-10 へ明文化 ⑤未使用CSS（`.at-link`）と重複していた対象フォルダ表記を削除 |
