...
3. 確定後、**Google 認証チェックポイント**（`auth-checkpoint.js`、v1.2）を通す。トークンが有効で残りが `SAVE_TOKEN_MIN_SECONDS`（10 分）以上ならそのまま次の順で進める（`save-flow.js`）。
   **認証確定 → 保存先フォルダを保証（`drive-folders.js`）→ Drive へアップロード → 台帳を `UPLOADED`（`driveFileId` 付き）→ 台帳を `PROCESSING` → 文字起こし・議事録 → Markdown を Potenitas record へ保存 → 全工程の成功を確認 → 台帳から削除 → OPFS の録音を削除。**
   足りなければ（未連携・期限切れ・残りが短い）台帳に残してホームへ戻し、理由に応じた案内を出す。「Driveへ保存」の押下（利用者操作の猶予がある文脈）で連携を更新してから同じ順で進む。90 分の録音中にトークンが失効しても、停止後の保存時に更新してから保存する。
...
**録音開始時に Google 連携は求めない**（v1.2。録音の必須条件にしない）。未連携なら録音画面に「Google 未連携のまま録音します。停止後に「Driveへ保存」から連携できます。」と出すだけで、連携の確認・更新は保存の直前に行う。
...
- 保存前チェックポイント（`auth-checkpoint.js` の `evaluateSaveAuth`）: 有効で残り 10 分以上なら OK、それ以外は利用者の押下で更新（`INSUFFICIENT` / `EXPIRED` / `NEVER_LINKED` で案内を分ける）。暗黙フローのため自動更新は無い。将来 Authorization Code Flow + PKCE + サーバー側 refresh token へ移行してもこの判定は流用できる。
- ポップアップが開かない理由を切り分ける: 要求時に `navigator.userActivation.isActive` が false（利用者操作の猶予なし）なら `OAUTH_USER_ACTION_REQUIRED`「連携の更新が必要です。「Driveへ保存」を押すと更新して続きます」、true なら `OAUTH_POPUP_BLOCKED`（本当のブロック解除の案内）。
...
保存先は毎回「保証」する（`drive-folders.js`、v1.2）。各階層を「名前と親」で検索し、無い階層だけ作る（`drive.js` の `ensureFolderChain`。同名が複数あれば createdTime が最も古いものを再利用し、二重に作らない）。解決した各階層の ID は `localStorage` `meeting-assistant-drive-folders` に保持し、次回は名前の逐次検索（4 往復）を省く。ただし保存前に各階層を `files.get` で**並列に**確認し、無い（404/403）・ゴミ箱・改名・親が違う（移動）なら捨てて解決しなおす。通信・認証の失敗は「無効」と扱わない（オフラインで作りなおして重複させないため）。「連携しなおす」で保持を捨てる。利用者にフォルダ作成の操作は求めない。

drive.file スコープの範囲: 見えるのは、このアプリ（同じ GCP プロジェクトの OAuth クライアント）が作成した、または利用者がピッカーで開いたファイル・フォルダだけ。実運用の 5 階層は 2026-08-23 に Worker 版（meeting.tsam-ai.com）が作成したもので、本アプリはそれを検索・再利用できている（同一プロジェクト）。利用者が Drive の画面で手作業で作った同名フォルダは検索に出ず、アプリは別のフォルダを作る（利用者からは重複に見える）。この制約は受容し、スコープは広げない。
