...
すべての操作は事業者側で行う。各手順に「所要」「戻し方」を書いてある。
...
- [ ] **Stripe 本番の領収書メールを有効にする**
  <https://dashboard.stripe.com/settings/emails>
  「Successful payments」をオンにする。テスト環境とは別管理。
...
| 症状 | 最初に見るところ |
| --- | --- |
| 決済は成立するが「支払済み」にならない | Webhookの登録URLの**末尾スラッシュ**。Stripeダッシュボードの該当エンドポイントで配信履歴を見る |
