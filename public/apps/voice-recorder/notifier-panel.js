...
/*
 * 通知用シートが記録したゲートの失敗（`/v1/vapid -> RATE_LIMITED` の形）を、
 * 「次に何をすればよいか」が分かる文にする。
 *
 * ------------------------------------------------------------------
 * 符号は括弧で残す
 ...
 * （2026-08-11）。運営には十分な情報だが、利用者には
 * **待てば直るのか、こちらの落ち度なのかが分からない。**
 *
 * かといって符号を消すと、問い合わせを受けたときの切り分けの起点が失われる。
 * 日本語の説明を主にし、符号は括弧で添える。
 * ------------------------------------------------------------------
 */
const GATE_ERROR_HINTS = Object.freeze({
  RATE_LIMITED: '通知サーバーへの問い合わせが立て込んでいます。'
    + '数分おいてから［接続テスト］をもう一度押してください。設定はこのままで構いません。',
  LICENSE_EXPIRED: 'ご契約が確認できないため、通知サーバーが鍵を発行していません。',
  NO_LICENSE: 'ご契約の情報がまだ通知用シートへ渡っていません。'
    + '［接続テスト］をもう一度押すと引き渡しをやり直します。',
  NETWORK: '通知用シートから通知サーバーへ接続できませんでした。時間をおいてお試しください。',
  BAD_PAYLOAD: '通知サーバーの応答を読み取れませんでした。お手数ですがサポートへお知らせください。',
  UNAUTHORIZED: 'ご契約の情報が通知サーバーで確認できませんでした。'
    + '［通知をセットアップ］からやり直してください。',
});

export function describeGateError(record) {
  ...
  const hint = GATE_ERROR_HINTS[code]
    ?? (code.startsWith('HTTP_5') ? '通知サーバーで問題が起きています。時間をおいてお試しください。' : '');

  return hint === ''
    ? `通知サーバーとのやり取りに失敗しています（${text}）。`
    : `${hint}（${text}）`;
}
...
async function runChecks() {
  ...
  try {
     ...
     */
    const gateHint = describeGateError(health.lastGateError);
