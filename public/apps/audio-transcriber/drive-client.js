/*
 * Google Drive API v3 クライアント（音声文字起こしアプリ用）。
 *
 * 担当するのは Drive API の呼び出しだけ。
 * DOM操作・認可フロー・画面文言はここに置かない。
 * 認可（アクセストークンの取得）は ../drive-auth.js を使う。複製しない。
 *
 * ------------------------------------------------------------------
 ...
 *
 * このアプリが読みに行くのは A である。
 * 音声録音アプリ（apps/voice-recorder/）は、このアプリと同じOAuthクライアント
 * （apps/auth-config.js の clientId）で、同じ ../drive-auth.js を使って
...
export async function resolveVoiceRecorderFolder({ token, signal, fetchImpl } = {}) {
  ...
  const rootId = await resolveSingleFolder(DRIVE_FOLDERS.root, 'root', {
  ...
  const folderId = await resolveSingleFolder(DRIVE_FOLDERS.voiceRecorder, rootId, {
...
export async function ensureTranscriptFolder(options) {
  const rootId = await ensureFolder(DRIVE_FOLDERS.root, null, options);
  return ensureFolder(DRIVE_FOLDERS.audioTranscriber, rootId, options);
}
