 ...
 * apps/ 配下は npm ビルドを持たない素の ES Modules で構成されており、
...
export async function transcribeWithGemini(blob, {
  ...
  const availableIds = await listUsableModels({ apiKey: key, signal });
  const modelOrder = resolveModelOrder({ preferredId: preferredModelId, availableIds });
