 ...
 * 共通の文言。画面ごとに書き分けない。
 *
 * ここに置くのは「画面側で完結する検証・案内」の文言だけにする。
 * サーバーが返すエラーの文言は Response.gs（GAS 側）が持ち、
 * 画面はそれをそのまま表示する。二重に定義しない。
 *
 * 通信失敗と API 未設定の文言は auth/api.js が持つ。
 * 例外を投げる場所と文言を同じファイルへ置くほうが、食い違いに気づきやすい。
 */
export const MESSAGES = Object.freeze({
  ...
  storageUnavailable: 'このブラウザではログイン状態を保持できません。プライベートモードを解除してお試しください。',
});
