...
/*
 * Gemini へ送る要求本文。
 * 事実の創作を防ぐ指示は、Threads 版（../threads-post/gemini.js）から引き継ぐ。
 */
export function buildPostRequest(theme, { maxOutputTokens = MAX_OUTPUT_TOKENS } = {}) {
  const prompt = 'あなたは、X（旧 Twitter）への投稿文を書く日本語のライターです。\n'
    + '以下のテーマ・指示をもとに、投稿文を1本作成してください。\n'
    + '\n'
    + '# 投稿の方針\n'
    + `- 日本語なら${WEIGHT_LIMIT / 2}文字以内にすること（Xの上限は半角${WEIGHT_LIMIT}相当で、全角1文字が2と数えられる）。\n`
