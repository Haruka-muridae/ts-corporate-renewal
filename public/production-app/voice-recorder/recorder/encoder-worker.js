/*
 * 逐次エンコード Worker（classic worker）。要件書 §FR-06 / §8.2。
 ...
 *
 * ------------------------------------------------------------------
 * public/apps/voice-recorder/long-encoder-worker.js からの複製
 * ------------------------------------------------------------------
 * 複製元との違い:
 *   - ビットレートを定数で持たず、init メッセージで受け取る
 *     （複製元は 96kbps 固定。本アプリは 128kbps だが、値の正は ../config.js
 *      1か所であるべきで、Worker 側に二つ目の定義を作らないため）
 *   - lamejs の読み込み先が ../vendor/（この Worker が recorder/ の下にあるため）
 * ------------------------------------------------------------------
 */
...
/* init で上書きされる。ここは受け取れなかった場合の保険（要件書 §FR-06 の値）。 */
let bitrateKbps = 128;
...
async function handleInit(message) {
  ...
  if (Number.isFinite(message.bitrateKbps) && message.bitrateKbps > 0) {
    bitrateKbps = message.bitrateKbps;
  }

  try {
    /* 第1引数の 1 はチャンネル数。モノラル固定（§FR-06）。 */
    encoder = new lamejs.Mp3Encoder(1, sampleRate, bitrateKbps);
  } catch (error) {
