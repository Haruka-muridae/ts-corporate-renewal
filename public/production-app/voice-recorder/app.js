 ...
 *
 * ------------------------------------------------------------------
 * 完了のままなら open へ代入しない（§7「正常と分かった時点でのみ畳む」）
 * ------------------------------------------------------------------
 * この関数は updateSaveButton() 経由で録音停止・破棄・保存のたびに呼ばれる。
 * 呼ばれるたびに open を書くと、利用者が自分で開いたパネルを次の操作が
 * 閉じ直してしまう。「連携しなおす」は #vr-connect にしかなく、それは
 * このパネルの中にあるため、開けても閉じられる状態では**連携をやり直す
 * 手段が画面から無くなる**（別アカウントへ切り替えたいなど、期限切れ
 * エラーではないので #vr-retry-actions の導線も出ない場面がある）。
 *
 * そこで open を触るのは状態が変わった時点だけにする。
 *   未完了 → 完了 : 畳む（§7 の「その時点でのみ」）
 *   完了   → 未完了: 開く（§7 の「1つでも問題があれば開いたまま」）
 *   変化なし        : 触らない（利用者の開閉を上書きしない）
 * ------------------------------------------------------------------
 */
let prepAllReady = null;
...
function updatePrepPanel() {
  ...
  if (allReady !== prepAllReady) {
    el['vr-prep'].open = !allReady;
    prepAllReady = allReady;
  }
}
