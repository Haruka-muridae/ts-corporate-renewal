/*
 * Google Drive API v3 の呼び出し（仕様書 §4）。
 *
 * ------------------------------------------------------------------
 * 移植元と複製の理由
 * ------------------------------------------------------------------
 * public/production-app/voice-recorder/drive.js からの複製（2026-08-19）。
 * 保存先フォルダ（マイドライブ ＞ TSAM AI ＞ Voice Recorder）も、名前から
 * 解決して無ければ作るという手順も、同名時の連番も、voice-recorder と
 * 同一でなければならない。**同じ場所を別の作法で触ると、片方が作った
 * フォルダをもう片方が見つけられず、同名フォルダが並ぶ。**
 *
 * import ではなく複製にしているのは、本番アプリ同士を相互参照しないという
 * 流儀（voice-recorder/config.js の DRIVE_NAMES コメント、
 * docs/repository-structure.md §1）による。複製元の該当処理を直すときは、
 * こちらも直すか「直さない」と判断すること。
 *
 * 複製元から削ったものは無い。fetchAccountEmail まで残しているのは、
 * 保存後に「どのアカウントのドライブへ入ったか」を出すためである
 * （面談録音は業務用と個人用のアカウントを取り違えると影響が大きい）。
 * ------------------------------------------------------------------
 *
 * 担当するのは Drive の操作だけ。DOM操作・画面文言・判断はここに置かない。
 * トークンは引数で受け取り、このモジュールでは保持しない。
 *
 * ------------------------------------------------------------------
 * 共有設定を一切付与しない
 * ------------------------------------------------------------------
 ...
 * 面談の音声という性質上、これは特に重要である。
 * ------------------------------------------------------------------
 *
 * 使うスコープは drive.file のみ。
 ...
 * Google の仕様で 256KB の倍数であることが要る。8MB なら長時間の面談でも
 * 十数回で終わり、1回あたりの再送も現実的な大きさに収まる。
 */
...
/*
 * 保存先「マイドライブ ＞ TSAM AI ＞ Voice Recorder」を解決する（§4）。
 *
...
/*
 * 保存先にある同名ファイルを避けた名前を返す（§4）。
 * `名前.mp3` が既にあれば `名前_2.mp3`、それもあれば `名前_3.mp3`…。
 *
 * drive.file スコープでは「このアプリが作成したファイル」しか見えない。
 * ここでの「このアプリ」はクライアントID単位なので、voice-recorder が
 * 保存した MP3 も同じIDである以上ここから見える（config.js の OAUTH コメント）。
 * 一方、利用者が手で置いた同名ファイルは検索に出てこないため、そのときは
 ...
 * MP3 を分割送信する。
 *
 * file は Blob でも File でもよい（slice / size しか使わない）。
 * 本アプリは逐次エンコードした MP3 を Blob として持っているため Blob を渡す。
 *
 * 1回の POST で 80MB 超を送ると、途中で切れたときに最初からやり直しになる。
 * 分割送信ならチャンク単位で状況が分かり、進捗も出せる。長い面談ほど
 * ここが効くため、複製元と同じく resumable のままにしてある。
 *
