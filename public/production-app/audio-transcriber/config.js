 ...
 * 音声文字起こしアプリ（本番）の設定値。
 *
 * public/apps/audio-transcriber/config.js からの複製・適合。
...
/* サイトのルートから見た、このアプリの深さ（production-app/audio-transcriber/ → 2）。 */
 ...
 * 「対象フォルダ以外の Google Drive データを読み取らない」方針が崩れる。
 *
 * クライアントIDは録音アプリ（production-app/voice-recorder/config.js）・
 * 領収書スキャナ（production-app/receipt-ocr/config.js）と同一のものを使う。
 * drive.file スコープで見えるのは「同じOAuthクライアントのアプリが作成した
 * ファイル」だけなので、**同じIDでないと録音アプリが保存したMP3が見えない。**
 * ここを別のIDへ変えると、このアプリの主要機能（録音の一覧・取得）が壊れる。
 *
 ...
 * IDは毎回「名前と親の関係」から解決する（drive-client.js）。
 *
 * root / voiceRecorder は録音アプリ側にも同名の定義がある
 * （production-app/voice-recorder/config.js の DRIVE_NAMES）。
 * 文字起こしアプリ／録音アプリの双方で同名定義があるため、
 * **片方だけ変えないこと。** 名前がずれると、同じ場所を指しているつもりで
 * 別のフォルダを見ることになる。
 * 複製であって import ではないのは、アプリ間で相互参照しないという
 * 本番アプリ群の流儀（voice-recorder 側の同コメントを参照）による。
 * ------------------------------------------------------------------
 */
export const DRIVE_NAMES = Object.freeze({
  ...
  /* TSAM AI 直下。録音アプリが録音（MP3）を保存する場所。読むだけで作らない。 */
  voiceRecorder: 'Voice Recorder',

  /* TSAM AI 直下。このアプリが結果のTXTを保存する場所。初回保存時に作成する。 */
...
export const DRIVE = Object.freeze({
  /*
   * Google Picker はこのアプリでは使わない（テスト版で残していた drive-picker.js は
   * 未使用のため本番へは複製していない）。Picker用のAPIキー・pickerAppId も持たない。
   * フォルダ名は上の DRIVE_NAMES で定義する。フォルダIDは持たない
   * （利用者ごとに異なるため）。
   */
