 ...
 *
 * production-app/voice-recorder/oauth.js（元をたどれば receipt-ocr/oauth.js）を
 * ベースにした自前実装。テスト環境の public/apps/drive-auth.js は参照しない
 * （本番アプリからテスト環境を参照しない境界 … docs/repository-structure.md §1）。
 *
 * ==================================================================
 * トークンはメモリだけ
 * ==================================================================
 ...
 * ==================================================================
 *
 * voice-recorder 版との違い（意図的な差分）:
 *   - エラーを DriveAuthError / DriveAuthErrorCode で返す。
 *     画面（script.js）がテスト版と同じ粒度でエラーを出し分けているため、
 *     その分類（POPUP_CLOSED / POPUP_BLOCKED / ACCESS_DENIED / SCOPE_NOT_GRANTED）
 *     を残した。voice-recorder は AppError（errors.js）へ写しているが、
 *     このアプリの文言表はこちらの分類を前提にしている。
 *   - ensureAccessToken({ forceConsent }) という入口。有効なトークンが
 *     あればポップアップを出さずに返す（テスト版 drive-auth.js と同じ契約）。
 *   - login_hint は使わない。本番系には /apps/ のGoogleログイン表示
 *     （IDトークン由来のプロフィール）が存在しないため、アカウント選択は
 *     Google の画面に任せる。
 *
 ...
 * 文言側で両方の可能性を案内する（script.js の AUTH_ERROR_MESSAGES）。
 * ------------------------------------------------------------------
 */
