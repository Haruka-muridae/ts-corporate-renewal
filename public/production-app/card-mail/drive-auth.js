 ...
 * 複製元
 * ==================================================================
 * ../card-ocr/drive-auth.js を複製（2026-08-10）。**import はしない**
 * （docs/repository-structure.md §4-1）。
 *
 * 複製元から変えたところ:
 *   - スコープを drive.file + gmail.send の2つにした（config.js の
 *     REQUIRED_SCOPES）。**2つとも付与されたことを検証する。**
 *     利用者は同意画面で片方だけチェックを外せるため、片方だけの
 *     トークンで進むと、読み込みは成功するのに送信で403になる。
 * ==================================================================
 *
 * ==================================================================
...
export function describeDriveAuthError(error) {
  ...
  switch (code) {
    ...
    case DriveAuthErrorCode.SCOPE_NOT_GRANTED:
      return {
        text: 'ドライブの読み取りとメール送信の両方を許可してください。チェックを外すと利用できません。',
        errorCode: 'OAUTH-001',
      };
...
/*
 * 付与されたスコープに**必要な2つがどちらも**含まれるかを確認する。
 *
 * 利用者は同意画面でチェックを個別に外せる。外されたまま進むと、
 * あとの API 呼び出しで 403 になって原因が分かりにくい。ここで弾く。
 *
 * GIS の hasGrantedAllScopes があればそれを使い、無ければ scope 文字列で判定する。
 */
export function hasRequiredScopes(response) {
  const oauth2 = globalThis.google?.accounts?.oauth2;

  if (typeof oauth2?.hasGrantedAllScopes === 'function') {
    try {
      return oauth2.hasGrantedAllScopes(response, ...REQUIRED_SCOPES);
    } catch {
