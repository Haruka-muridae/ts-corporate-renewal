 ...
 * 複製元
 * ==================================================================
 * public/production-app/card-ocr/poc/gis-loader.js を複製（2026-08-04）。
 * その元は public/apps/gis-loader.js。**どちらも import はしない。**
 *
 * `/apps/` はテスト環境であり、本番アプリからそこへ依存を作らない。
 * 本番アプリどうしでも共通層を作らず複製する
 * （docs/repository-structure.md §4-1）。
 *
 * PoC からの変更:
 *   - 設定の参照先を ./config.js に変えた
 *   - 構造（Promise を1つだけ持つ、失敗時にキャッシュを捨てる）は同じ
 * ==================================================================
 *
 * ==================================================================
 * 失敗したキャッシュを残さないこと
 ...
 * 連携が二度と成功しない。** 案内が「もう一度お試しください」でも、
 * そのとおりにして直らない状態になる。
 *
 * 領収書OCRの oauth.js は現にそうなっている
 * （docs/receipt-ocr-findings-20260804.md #1）。同じ形にしない。
 * ==================================================================
 *
