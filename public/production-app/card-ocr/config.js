...
/*
 * card-ocr のために新規発行したクライアントID（2026-08-03）。
 *
 * テスト環境 `/apps/` が使う既存IDとも、領収書OCRが使うIDとも
 * **別のクライアント**である（フェーズ0計画 §6-2 の決定）。
 *
 * ==================================================================
 * ただし card-mail とは意図的に共用している（訂正 2026-08-18）
 * ==================================================================
 * 名刺メール配信アプリ（`../card-mail/`）が**同じIDを使う。**
 * drive.file は「そのクライアントIDで作成したファイルだけ」を見せる
 * 権限なので、**IDを分けると card-mail から台帳「名刺管理」が見えない。**
 * 台帳を読むことがあちらの目的そのものなので、例外として共用する
 * （ルート CLAUDE.md「名刺メール配信アプリ」節、card-mail の config.js）。
 *
 * したがって「このIDで作ったファイルが見えるのはこのアプリだけ」では
 * ない。**card-mail からも同じ台帳が見える。** 以前ここには
 * 「card-ocr 専用」と書いてあったが、実態と食い違っていたので直した。
 *
 * 共用の代償として、**このクライアントの OAuth 設定を変えると
 * card-mail にも及ぶ**（card-mail は gmail.send を足す予定。同アプリの
 * config.js）。片方の都合で生成元やスコープをいじらないこと。
 * ==================================================================
 *
