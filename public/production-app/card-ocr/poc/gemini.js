...
/*
 * ==================================================================
 * モデルの選定（確定。要件定義書 v3.2 §FR-11 / §20）
 * ==================================================================
 * 当初の主モデルは `gemini-3.5-flash-lite` だったが、503（混雑）が
 * 続いた。主とフォールバックを入れ替えたところ、**同じキー・同じ
 * オリジン・同じリクエストで6/6成功した**（計画 §7-5-2）。
 * 503 はリクエストの形やキーの問題ではなく、そのモデルの空き具合
 * だったと実証できたので、**実測で通ったほうを主に置く。**
 *
 * 両モデルとも実在することは確認済み。したがって下のフォールバックが
 * 働く場面は「**主モデルが廃止されたとき**」であり、混雑時の退避では
 * ない。混雑で別モデルへ逃がすかどうかは §20 の要判断として残っている
 * （退避先では精度が変わるため、同じ名刺でも日によって結果が違う）。
 * ==================================================================
 */
 ...
 * 切り替えの条件にしている。上の注記のとおり、これは廃止への備えである。
 */
