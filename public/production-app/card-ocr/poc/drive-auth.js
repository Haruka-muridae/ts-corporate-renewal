 ...
 * 複製元
 * ==================================================================
 * public/apps/drive-auth.js を複製・簡略化したもの。**import はしない。**
 * `/apps/` はテスト環境であり、本番アプリからそこへ依存を作らない
 * （docs/repository-structure.md §2-1、要件定義書 §3）。
 *
 * 複製元から落としたもの:
 *   - login_hint（`/apps/` のログイン表示に依存するため。本アプリは
 *     TSAM AI のセッションで認証しており、Google のプロフィールは持たない）
 *   - ロガーの差し替え機構（検証ページには不要）
 *
 * 複製元から変えていないもの:
 *   - スコープは drive.file 1つだけ
 *   - トークンはメモリのみ。Storage へ書かない
 *   - 付与スコープの検証（hasGrantedAllScopes → 文字列判定の順）
 * ==================================================================
 *
 * ==================================================================
 * 方針（要件定義書 §FR-02 の2、§FR-24、§14.2）
 * ==================================================================
 ...
 *   - 当社サーバーへ送らない（要件定義書 §12）。
 *   - クライアントシークレットも refresh token も使わない。
 *     静的サイトに置けないため。
 * ==================================================================
 */

import { DRIVE_SCOPE, GOOGLE_CLIENT_ID, isClientIdConfigured } from './google-config.js';
 ...
 *   clientId     … 既定は google-config.js の値。テストから差し替える口
 */
