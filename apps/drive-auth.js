...
/*
 * このファイルは複数のアプリから使うため、特定アプリのデバッグ機構に依存しない。
 * 既定では何も出力せず、必要なアプリが setDriveAuthLogger() で差し込む。
 *
 * logger へ渡してよいのは要約だけ。
 * アクセストークン・IDトークン・メールアドレスは渡さない（呼び出し側の責任）。
 */
let logger = () => {};

export function setDriveAuthLogger(fn) {
...
function createTokenClient(hint) {
  ...
  return oauth2.initTokenClient({
    ...
    callback: (response) => {
      ...
      logger('auth:token-acquired', { tokenPresent: true, expiresInSec: Math.round(lifetimeMs / 1000) });
    ...
    error_callback: (error) => {
      ...
      logger('auth:error-callback', { type: error?.type ?? 'unknown' });
...
export async function ensureAccessToken({ forceConsent = false } = {}) {
  ...
  if (!tokenClient || tokenClientHint !== hint) {
    ...
    logger('auth:token-client', { hintPresent: hint !== undefined });
