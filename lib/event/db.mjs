...
/**
 * 支払済みの申込を「受付番号と氏名だけ」取る（カレンダーへ書き戻す名簿）。
 *
 * 取る列を2つに絞っているのは、カレンダーの説明欄という
 * アプリの外へ出す先に個人情報を持ち出すため。当日の受付で照合できる
 * 最小限（受付番号・氏名）に限り、フリガナ・メール・電話・会社名は取らない。
 * 「表示側で捨てる」のではなく「そもそも取得しない」ことで、
 * 別の用途へ流用されにくくしてある。
 *
 * 並びは受付番号の昇順。受付番号は支払済みになった順に発番される
 * ゼロ埋めの連番（TSAM-0001）なので、文字列順がそのまま申込順になる。
 * 受付番号が無い行（DBを直接触った等の例外）は Postgres の既定で末尾に来る。
 *
 * 件数そのものは countPaidApplications（DB側で数える正確な件数）を使う。
 * ここは名簿の材料で、件数の正はこちらに移さない。
 *
 * @param {object} config
 * @param {string} eventId
 * @returns {Promise<{ receipt_number: string | null, name: string }[]>}
 */
export async function listPaidAttendees(config, eventId) {
  const rows = await request(
    config,
    `applications?event_id=eq.${encodeURIComponent(eventId)}&status=eq.paid`
    + '&select=receipt_number,name&order=receipt_number.asc',
  );
