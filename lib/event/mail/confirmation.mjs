...
/*
 * 件名など、短く済ませたい箇所向けの「2026年8月30日」形式（時刻を含まない）。
 * 開催日が複数あるため、件名にどの回への確認かを示すために使う。
 */
export function formatEventDateLabel(startAt) {
  const parts = new Intl.DateTimeFormat('ja-JP', {
    timeZone: 'Asia/Tokyo',
    year: 'numeric',
    month: 'numeric',
...
export function buildConfirmationMail({ event, application, payment }) {
  ...
  if (discountLines.length > 0) {
    /*
     * 桁を揃えるための空白は入れない。メールソフトの多くは等幅で表示しないため、
     * 空白で揃えても崩れて見える。区切りは「：」に統一する。
     */
    lines.push('適用された割引');
  ...
  /*
   * 開催日が複数あるため、件名にもどの回かを添える。
   * 本文は開催日時をすでに載せているので、件名側は日付だけの短い表記にする。
   */
  const dateLabel = formatEventDateLabel(event.startAt);

  return {
    subject: `【${event.name}（${dateLabel}開催）】お申し込みを承りました（受付番号 ${application.receiptNumber}）`,
    text: lines.join('\n'),
  };
}
