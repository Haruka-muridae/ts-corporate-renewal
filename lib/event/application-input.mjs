...
/*
 * events.id は UUID。フォームのラジオ値がそのまま届くため、
 * 形式だけここで確かめる（実在するか・公開中かはサーバー側で別途確認する）。
 */
 ...
 * UUIDの形式かどうか。
 *
 * 申込フォーム以外（管理画面のCSVなど、URLの ?eventId= を受け取る箇所）でも
 * 同じ判定を使う。形だけの検査で、実在するかは呼び出し側がDBで確かめる。
 * 形の合わない値をそのまま問い合わせに載せると、PostgREST 側のエラー文言や
 * 予期しない絞り込みの挙動に依存することになるため、手前で弾く。
 *
 * @param {unknown} value
 * @returns {boolean}
 */
...
/*
 * 参加日（events.id）。未選択・不正な形式のどちらも同じ文言にする。
 * 「どのUUIDが無効か」を利用者に伝える意味はなく、選び直してもらえば済むため。
 */
function checkEventId(errors, value) {
  if (!isUuid(value)) {
    errors.eventId = '参加日を選択してください';
  }
}
...
export function validateApplicationInput(raw) {
  ...
  const eventId = trim(raw.eventId);
  checkEventId(errors, eventId);
