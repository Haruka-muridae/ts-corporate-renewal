 ...
 *   - カレンダーへの人数の書き戻しも同じ扱い。失敗しても結果の文字列に
 *     残すだけで、支払の記録もメールも巻き戻さない（外部サービスの都合で
 *     成立済みの決済を壊さない）。
 *   - 依存（DB・メール送信・カレンダー書き戻し）は引数で受け取る。
 *     テストで差し替えられるようにする。
 * ==================================================================
 */

import { updateAttendeeNote } from './calendar-note.mjs';
 ...
 */
async function markPaid({ config, db, mailer, calendarNote, session }) {
  ...
  /*
   * 席が1つ埋まった時点の人数と名簿をカレンダーへ書き戻す。
   * 数えるのは applications.status を 'paid' にした後（この申込を含める）。
   * 失敗しても例外にならない（updateAttendeeNote が文字列で返す）。
   */
  const noteResult = await updateAttendeeNote({
    config, db, writer: calendarNote, eventId: application.event_id,
  });

  return `支払済みに更新（受付番号 ${receiptNumber}）／${mailResult}／${noteResult}`;
}
 ...
 * 反映するだけ（仕様書7.1、受入条件9）。
 */
async function markRefunded({ config, db, calendarNote, charge }) {
  ...
  /*
   * 返金で status が 'refunded' になり、支払済みの件数から外れる（席が空く）。
   * カレンダーの人数もその後の値に合わせる。
   */
  const noteResult = await updateAttendeeNote({
    config, db, writer: calendarNote, applicationId: payment.application_id,
  });

  return `返金済み（例外対応）に更新しました／${noteResult}`;
}
 ...
 *   calendarNote?: { write: (input: object) => Promise<unknown> } | null,
 * }} input
 * @returns {Promise<{ handled: boolean, duplicate: boolean, result: string }>}
 */
export async function handleStripeEvent({ event, config, db, mailer, calendarNote = null }) {
    ...
    switch (event.type) {
      case 'checkout.session.completed':
        ...
        result = object.payment_status === 'paid'
          ? await markPaid({ config, db, mailer, calendarNote, session: object })
          : `支払未確定のため待機します（payment_status=${object.payment_status}）`;
        break;

      case 'checkout.session.async_payment_succeeded':
        result = await markPaid({ config, db, mailer, calendarNote, session: object });
        break;
      ...
      case 'charge.refunded':
        result = await markRefunded({ config, db, calendarNote, charge: object });
        break;
